
#pragma once
#include "Client_Defines.h"
#include "FSM.h"

NS_BEGIN(Client)

class LittleMonster_RIGHT_WALK_State : public State
{
private:
    LittleMonster_RIGHT_WALK_State();
public:
    virtual ~LittleMonster_RIGHT_WALK_State();
public:
    HRESULT Initialize();
public:
    virtual void Enter(FSM* pFSM);
    virtual void Exit(FSM* pFSM);

    virtual void Priority_Update(FSM* pFSM, float fDeltaTime);

    virtual void Update(FSM* pFSM, float fDeltaTime);

    virtual void Late_Update(FSM* pFSM, float fDeltaTime);


public:
    static  shared_ptr<LittleMonster_RIGHT_WALK_State> Create();

};

NS_END
