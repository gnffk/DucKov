...
HRESULT BossMonster_Page2FSM::Initialize(uint32_t* eModelState)
{
	...
	__super::AddState(BOSS_STATE::HIT, Boss_HIT_State::Create());
...
void BossMonster_Page2FSM::Change_State(BOSS_STATE eState)
{
	// 이미 같은 상태면 무시
...
shared_ptr<BossMonster_Page2FSM> BossMonster_Page2FSM::Create(uint32_t* eModelState)
{
	auto	pInstance = shared_ptr<BossMonster_Page2FSM>(new BossMonster_Page2FSM());
