...
public:
	HRESULT Initialize(uint32_t materialtype, vector<vector<TEXTUREINFO>>& textureTypes, const _char* ModelFilePath);
...
public:
	static shared_ptr<Material> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, 
		uint32_t materialtype, vector<vector<TEXTUREINFO>>& textureTypes, const _char* ModelFilePath);
};
